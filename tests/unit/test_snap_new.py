...
class TestSnap(unittest.TestCase):
    ...
    def test_generate_reply_error_3(self, mock_dev,mock_tostring, mock_err, mock_cli, mock_write_warn):
        ...
        test_file = open(test_file, 'r')
        test_file = yaml.load(test_file)
        dev = Device(host='10.221.136.250', user='regress', passwd='MaRtInI')
        dev.open()
    ...
    @patch('jnpr.jsnapy.sqlite_store.JsnapSqlite.insert_data')
    @patch('jnpr.jsnapy.snap.JsnapSqlite')
    def test_write_warning(self, mock_sqlite, mock_insert_data):
